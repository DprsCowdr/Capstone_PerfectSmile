...
class Dentist extends BaseController
{
    ...
    {
        // Prefer reading the session directly for AJAX requests (more robust across contexts)
        $sess = session();
        $sessUserType = $sess->get('user_type');
        $sessUserId = $sess->get('user_id');
        if (!in_array($sessUserType, ['dentist', 'doctor', 'admin'], true)) {
            log_message('warning', 'Dentist::getAppointmentDetails unauthorized. sess_user_id=' . ($sessUserId ?? 'none') . ' sess_user_type=' . ($sessUserType ?? 'none'));
    ...
    {
        // Accept both 'dentist' and legacy 'doctor' roles, and allow admin system calls.
        // For AJAX calls it's more robust to read the session directly.
        $sess = session();
        $sessUserType = $sess->get('user_type');
        $sessUserId = $sess->get('user_id');
        if (!in_array($sessUserType, ['dentist', 'doctor', 'admin'], true)) {
        ...
        try {
            ...
            log_message('error', 'Dentist::getAppointmentDetails exception: ' . $e->getMessage());
